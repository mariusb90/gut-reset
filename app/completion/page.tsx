...
import { getAllLocalLogs, getLocalBaseline } from '@/lib/storage';
...
export default function CompletionPage() {
  ...
  const { startDate, configuredSupplements, goals } = useAppStore();
  ...
  const suppAdherence = (() => {
    const days = logs.filter(l => l.evening_checked_in && l.supplements_taken);
    if (!days.length || !configuredSupplements.length) return null;
    const totalSlots = days.length * configuredSupplements.length;
    const taken = days.reduce((acc, l) => {
      return acc + configuredSupplements.filter(k => l.supplements_taken?.[k]).length;
    }, 0);
  ...
  const mealCompliance = (() => {
    ...
    // We don't have meal data on each log object directly, so compute from all logs
    const total = completedDays * SLOTS.length;
    if (total === 0) return null;
    // Gut score proxy: estimate from gut scores
    return null; // will gracefully skip in UI if null
  })();
